...
namespace Otrabotka.Level
{
    ...
    public class ChunkSpawner : ManagerBase
    {
        ...
        private void SpawnAt(int index)
        {
            ...
            {
                // Exit map
                if (transport.HasExitForChunk(index))
                {
                    var exitInfo = transport.GetExitInfo(index);
                    var prefab = exitInfo.definition.exitEventPrefab;
                    if (prefab != null)
                    {
                        var exitGO = Instantiate(prefab, Vector3.zero, Quaternion.identity, container.transform);
                        exitGO.name = prefab.name;
                ...
                // Entry map
                if (transport.HasEntryForChunk(index))
                {
                    var entryInfo = transport.GetEntryInfo(index);
                    var prefab = entryInfo.prefab;
                    if (prefab != null)
                    {
                        var entryGO = Instantiate(prefab, Vector3.zero, Quaternion.identity, container.transform);
                        entryGO.name = prefab.name;
                        entryGO.transform.localPosition = entryInfo.localPosition;
                        entryGO.SetActive(true);
                        foreach (var tr in entryGO.GetComponentsInChildren<Transform>(true))
                            tr.gameObject.SetActive(true);
                    }
                }
            }
